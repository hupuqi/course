						...
						<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' name='ids' value="${r"${var."}<#list fieldList as var><#if var[5]=='PRI'>${var[4]}</#if></#list>${r"}"}" /><span class="lbl"></span></label>
								</td>
								...
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											...
											<li><a style="cursor:pointer;" title="编辑" onclick="edit('${r"${var."}<#list fieldList as var><#if var[5]=='PRI'>${var[4]}</#if></#list>${r"}"}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:if test="${r"${QX.del == 1 }"}">
											<li><a style="cursor:pointer;" title="删除" onclick="del('${r"${var."}<#list fieldList as var><#if var[5]=='PRI'>${var[4]}</#if></#list>${r"}"}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
											</c:if>
										</ul>
										</div>
									</div>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>${objectNameLower}/del.do?<#list fieldList as var><#if var[5]=='PRI'>${var[0]}</#if></#list>="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>${objectNameLower}/goEdit.do?<#list fieldList as var><#if var[5]=='PRI'>${var[0]}</#if></#list>='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
